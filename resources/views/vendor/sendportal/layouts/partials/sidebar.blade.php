<div class="sidebar-inner mx-3">
    <ul class="nav flex-column mt-4">
        ...
        <!-- ATALHO DIRETO: Resend API -->
        <li class="nav-item {{ request()->routeIs('resend.keys') || request()->is('*email-services/create*') || request()->is('*email-services/*/edit*') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('resend.keys') }}">
                <i class="fa-fw fas fa-key mr-2"></i><span>Resend API</span>
            </a>
        </li>

        <li class="nav-item {{ request()->is('*email-services') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('sendportal.email_services.index') }}">
                <i class="fa-fw fas fa-server mr-2"></i><span>{{ __('Email Services') }}</span>
            </a>
        </li>
